...
public abstract class AbstractWall extends AbstractGameObject {
	...
	public AbstractWall (Integer x,Integer y,Integer width,Integer height,Integer corner){
		this.y=y;
		this.x=x;
		this.width=width;
		this.height=height;
	...
	public Polygon getBounds(){
		...
		p.addPoint(((this.height/2)+this.x)-(this.width/2),(this.height/2)+this.y+(this.width/2));
		p.addPoint((this.height/2)+this.x,(this.height/2)+this.y);}
		
		else if(this.corner==135){
			p.addPoint(this.x, this.y);
			p.addPoint((this.x+this.width/2),this.y+(this.width/2));
			p.addPoint((this.x-(this.height/2))+(this.width/2),(this.height/2)+this.y+(this.width/2));

...
public class EditorMapPanel extends JPanel{
	...
	private int widthmappa ;
	private int  heightMappa ;
	...
	public EditorMapPanel() {
		...
		imageCubo0=ResourcesLoader.actor_mount.get(0).getScaledInstance(width, height, 0);
		muro0=new JButtonCustom(imageCubo0, imageCubo0, imageCubo0);
		imageCubo1=ResourcesLoader.actor_mount.get(1).getScaledInstance(width, height, 0);
		...
		muro0.setBounds(distanzaLeft, 220,width,50);
		...
		header.setBounds(distanzaLeft+100,30,800,50);
		setVisible(true);
		repaint();
		listenerLoader();
		addMouseListener(new MouseListener() {
			...
			if(rectangle.contains(e.getX(), e.getY()) && state!= 0){
				if(state==-1)
					nuovoMuro=new VerySquareWall(e.getX(), e.getY(), -1, imageCubo0);
				if(state==2)
						nuovoMuro=new VerySquareWall(e.getX(), e.getY(), 2, imageCubo1);
...
private void listenerLoader(){
	...
	save.addActionListener(e->{
			ManagerFilePlayer.saveMap(attrito,wall);
			Chiudere();
	});
}

...
public class Client implements Runnable {
	...
	public Client(String hostname) {
		...
		this.character = new OnlineCharacter(Player.getUsername(), new Point(0,
				0), ManagerFilePlayer.getTop(), ManagerFilePlayer.getMid(),
				ManagerFilePlayer.getBot());
	...
	public boolean connect() {
		try {
			...
			this.socket.connect(new InetSocketAddress(this.hostname, port),
					3000);
			this.protocol = new NetworkProtocol(this.socket.getInputStream(),
			...
			System.err
					.println("Error with server connection I/O on initialization");
	...
	public void run() {
		...
		while (!this.socket.isClosed()
				&& !ServerStatus.STOP.equals(Server.status)) {
			try {
				this.getInput();
				Object response = this.protocol.request();
				if (response instanceof CharacterMessage) {
					CharacterMessage message = (CharacterMessage) response;
					...
					if ((Boolean) response) {
						CollisionHandler.playCollision();
					}
				}
				GameWorld.update();
				...
				System.err
						.println("Error with server connection I/O on update");
				GameFrame.instance().showOnlineError("disconnected");
				protocol.close(socket);
				return;
	...
	public void syncCharacter() throws IOException, ClassNotFoundException {
		...
		this.character.getOrigin().move(
				GameWorld.getArena().getSpawn().get(this.client_id - 1).x,
	...
	public void syncOpponents() throws ClassNotFoundException, IOException {
		...
		for (int i = 1; i < maxOpponents; i++) {
			OnlineCharacter opponent = (OnlineCharacter) this.protocol
					.request();
	...
	public boolean whoWon(List<Entity> s) {
		...
		for (int i = 0; i < GameWorld.getObjects().size(); i++) {
			if (GameWorld.getObjects().get(i) instanceof OnlineCharacter) {
				OnlineCharacter couldWin = (OnlineCharacter) GameWorld
						.getObjects().get(i);

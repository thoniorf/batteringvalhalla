...
public class Actor extends AbstractMovableActor {
	...
	public Actor(int x, int y, int headId, int weapId, int mountId) {
		...
		this.mount = new Sprite(ResourcesLoader.actor_mount.get(mountId), ResourcesLoader.actor_mount.get(mountId).getWidth(null),
				ResourcesLoader.actor_mount.get(mountId).getHeight(null), 117, 88, 3, 16, 110 / 2, 110 / 2, 88 / 2);
		this.weapon = new Sprite(ResourcesLoader.actor_weapon.get(weapId),
				ResourcesLoader.actor_weapon.get(weapId).getWidth(null), ResourcesLoader.actor_weapon.get(weapId).getHeight(null),
